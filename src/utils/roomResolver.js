 ...
 * @param {any} profile
 * @returns {{ uid: string; displayName: string; photoURL: string | null; elo: number }}
 */
export function getHydratedProfile(profile) {
 ...
 * 1. Direct Document Key: doc(db, 'rooms', cleanCode) & case variants
 * 2. Indexed Field Query: collection(db, 'rooms') where roomCode/roomId/code == cleanCode
 ...
 * @param {any} [targetInvite]
 * @returns {Promise<{ room: any; tier: 1|2|3|4|5|6; tierName: string; source: string; isOnlineMatch?: boolean; gameId?: string; alreadyStarted?: boolean } | null>}
 */
export async function resolveRoom(rawCode, userProfile, targetInvite) {
  const cleanCode = normalizeRoomCode(rawCode || targetInvite?.roomCode || targetInvite?.roomId || targetInvite?.code);
  if (!cleanCode) return null;

  const activeProfile = getHydratedProfile(userProfile);

  // =========================================================================
  // Tier 1: Direct Document Key lookup (Direct Document Key)
  // =========================================================================
  try {
  ...
  // =========================================================================
  try {
    const fieldQueries = ['roomCode', 'roomId', 'code'];
  ...
  try {
    const storageKeys = [
      `chess_room_${cleanCode}`,
      `chess_room_${cleanCode.toLowerCase()}`,
    ];

    for (const sKey of storageKeys) {
      const cachedStr = localStorage.getItem(sKey);
      if (cachedStr) {
        const cached = JSON.parse(cachedStr);
        if (
          cached &&
          (normalizeRoomCode(cached.roomCode) === cleanCode ||
            normalizeRoomCode(cached.roomId) === cleanCode)
        ) {
          return {
            room: { ...cached, roomCode: cleanCode },
            tier: 3,
            tierName: 'Local Storage Cache',
            source: `localStorage[${sKey}]`,
  ...
  // =========================================================================
  // Tier 5: Server-Side Fallback (/api/rooms/join/${cleanCode})
  // =========================================================================
  try {
