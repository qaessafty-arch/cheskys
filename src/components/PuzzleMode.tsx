...
export const PuzzleMode: React.FC<PuzzleModeProps> = ({ settings }) => {
        ...
        if (nextStep >= currentPuzzle.solutionMoves.length) {
          ...
          try {
            confetti({ 
              ...
              spread: 70, 
              origin: { y: 0.6 },
              colors: ['#60a5fa', '#2563eb', '#1e3a8a', '#FFFFFF']
            });
          } catch {}
        } else {
  ...
  return (
    <PanelContainer>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 glass-panel p-3 sm:p-4 rounded-3xl border border-blue-400/30 shadow-xl">
        ...
        {/* Tab Switcher */}
        <div className="flex items-center gap-1.5 bg-[#161c12] p-1.5 rounded-2xl border border-blue-400/30 w-full sm:w-auto">
          <button
            type="button"
            ...
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-xs font-black transition-all cursor-pointer ${
              activeTab === 'daily'
                ? 'bg-gradient-to-r from-blue-800 to-blue-600 text-white border border-blue-400/50 shadow-md'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 flex flex-col items-center justify-center">
          <div className="relative p-2.5 sm:p-3.5 rounded-3xl bg-[#10140e] border-2 border-blue-400/30 shadow-2xl">
            ...
            <div className="mt-3 px-4 py-2 rounded-2xl bg-black/80 backdrop-blur-md border border-blue-400/30 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2">
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          {activeTab === 'daily' ? (
            <div className="glass-panel p-5 rounded-3xl border border-blue-400/40 shadow-xl relative overflow-hidden">
              ...
              <div className="flex items-center justify-between pb-3 border-b border-white/10 mb-3">
                ...
                <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-950/20 border border-blue-500/40 text-blue-300 text-xs font-black shadow-sm">
              ...
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                <span className="px-2.5 py-0.5 rounded-lg bg-[#8C2425]/40 text-blue-400 text-[11px] font-black border border-blue-400/30">
                  {dailyPuzzle.difficulty} ({dailyPuzzle.rating} Elo)
                </span>
                <span className="px-2.5 py-0.5 rounded-lg bg-blue-600/40 text-blue-300 text-[11px] font-bold border border-blue-500/30">
                  {dailyPuzzle.theme}
                </span>
              </div>

              <h3 className="text-lg font-black text-white mb-1.5">{dailyPuzzle.title}</h3>
              <p className="text-xs text-blue-100/80 mb-3 leading-relaxed">{dailyPuzzle.description}</p>

              {/* Kurdish Lore Context Card */}
              {dailyPuzzle.loreContext && (
                <div className="p-3 rounded-2xl bg-white/[0.04] border border-blue-400/20 text-xs text-blue-100/90 mb-3 flex items-start gap-2">
                  <span className="text-sm shrink-0">🏔️</span>
                  <div>
                    <span className="font-bold text-blue-400">Zagros Lore: </span>
              ...
              {puzzleStatus === 'completed' && (
                <div className="p-3.5 rounded-2xl bg-emerald-500/20 border border-emerald-500/50 text-emerald-300 flex items-center justify-between gap-3 mb-3 animate-in zoom-in-95">
                  ...
                  >
                    {dailyCopied ? <Check className="w-4 h-4 text-blue-300" /> : <Share2 className="w-4 h-4" />}
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                {puzzleStatus === 'wrong' ? (
                  <button
                    type="button"
                    onClick={handleRetry}
                    className="col-span-2 py-3 rounded-2xl bg-gradient-to-r from-[#8C2425] to-[#52673A] text-white font-black text-xs flex items-center justify-center gap-2 hover:scale-[1.01] transition-all cursor-pointer shadow-lg border border-blue-400/40"
                  >
            ...
            <div className="glass-panel p-5 rounded-3xl border border-blue-400/40 shadow-xl space-y-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className="px-2 py-0.5 rounded-md bg-[#8C2425]/30 text-blue-400 text-[11px] font-bold border border-blue-400/30">
                  {libraryPuzzle.difficulty}
                </span>
                <span className="px-2 py-0.5 rounded-md bg-blue-600/30 text-blue-300 text-[11px] font-bold border border-blue-500/30">
              ...
              <p className="text-xs text-blue-100/80 leading-relaxed">{libraryPuzzle.description}</p>
              ...
              {showHint && (
                <div className="p-3 rounded-2xl bg-blue-400/15 border border-blue-400/30 text-blue-200 text-xs flex items-start gap-2">
                  <Lightbulb className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
                  <div>
          ...
          <div className="glass-panel p-4 rounded-3xl border border-white/10 grid grid-cols-3 gap-2 text-center">
            <div className="p-2 rounded-2xl bg-white/[0.03]">
              ...
              <div className="text-[10px] text-blue-100/60 uppercase font-semibold">Daily Streak</div>
            </div>
            ...
            <div className="p-2 rounded-2xl bg-white/[0.03]">
              ...
              <div className="text-[10px] text-blue-100/60 uppercase font-semibold">Solved</div>
            </div>
            ...
            <div className="p-2 rounded-2xl bg-white/[0.03]">
              ...
              <div className="text-[10px] text-blue-100/60 uppercase font-semibold">Respect Pts</div>
            </div>
          </div>
        </div>
      </div>
