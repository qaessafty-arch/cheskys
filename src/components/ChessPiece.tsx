...
export const ChessPiece: React.FC<ChessPieceProps> = ({
  ...
  if (theme === 'one-piece') {
    const getOnePieceEmoji = () => {
      if (isWhite) {
        switch (type) {
          case "k": return "☀️"; // Luffy Gear 5 Nika
          case "q": return "🌩️"; // Nami Weather Egg
          case "b": return "⚔️"; // Zoro/Sanji
          case "n": return "🦁"; // Thousand Sunny
          case "r": return "🏯"; // Wano Castle
          case "p": return "🏴‍☠️"; // Grand Fleet
          default: return "";
        }
      } else {
        switch (type) {
          case "k": return "🌋"; // Akainu
          case "q": return "✨"; // Kizaru
          case "b": return "☄️"; // Fujitora
          case "n": return "🤖"; // Pacifista
          case "r": return "🏛️"; // Marineford
          case "p": return "⚓"; // Marine Soldier
          default: return "";
        }
      }
    };
    return (
      <div className={`relative w-full h-full flex items-center justify-center select-none pointer-events-none ${className}`}>
        <span className="text-[2.5rem] leading-none filter drop-shadow-[0_4px_6px_rgba(0,0,0,0.6)] hover:scale-110 transition-transform">{getOnePieceEmoji()}</span>
      </div>
    );
  }
